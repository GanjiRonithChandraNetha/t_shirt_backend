...
CREATE TABLE users (
    ...
    anonymous_count     BIGINT      NOT NULL DEFAULT 0
);
...
CREATE TABLE followers (
    ...
    user_min BIGINT GENERATED ALWAYS AS (LEAST(followee, follower)) STORED,
    user_max BIGINT GENERATED ALWAYS AS (GREATEST(followee, follower)) STORED,
    ...
    CONSTRAINT unique_follow_pair_min_max UNIQUE (user_min, user_max)
);
...
CREATE TABLE signatures (
    ...
    CONSTRAINT unique_named_signature_per_author UNIQUE (user_id, author_id)
...
CREATE INDEX idx_signatures_author           ON signatures(author_id);
...
-- Triggers
CREATE OR REPLACE FUNCTION decrement_anonymous_count()
RETURNS TRIGGER AS $$
BEGIN
  IF OLD.type = 'anonymous' THEN
    UPDATE users
    SET anonymous_count = GREATEST(anonymous_count - 1, 0)
...
CREATE TRIGGER trg_decrement_anonymous
AFTER DELETE ON signatures
FOR EACH ROW
EXECUTE FUNCTION decrement_anonymous_count();
